...
// upload scan from the empty note page -> ocr -> opens the new note
export default function BrainHandwrittenUpload({ classId }) {
  ...
  async function handleFileChange(e) {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['brains', classId, 'nodes'] });
      await queryClient.invalidateQueries({ queryKey: brainKeys.sources(classId) });
      setPhase('idle');
      navigate(`/class/${classId}/notes/${nodeId}`);
    } catch (err) {
  ...
  return (
    <section className="panel-rgb class-handwritten-upload">
      <h2 className="class-handwritten-upload-title">New note from a scan</h2>
      <p className="class-handwritten-upload-desc">
      ...
      {error ? <p className="class-handwritten-upload-error">{error}</p> : null}
    </section>
  );
}
